...
describe("Colors", () => {
  ...
  it("should have card as semi-transparent (rgba) in light mode", () => {
    expect(Colors.light.card).toMatch(/^rgba\(/);
  });

  it("should have card as semi-transparent (rgba) in dark mode", () => {
    expect(Colors.dark.card).toMatch(/^rgba\(/);
  });

  it("should have textTertiary lighter than textSecondary in light mode", () => {
    const t2 = parseFloat(Colors.light.textSecondary.match(/[\d.]+\)$/)![0]);
    const t3 = parseFloat(Colors.light.textTertiary.match(/[\d.]+\)$/)![0]);
    expect(t3).toBeLessThan(t2);
  });

  it("should have cardSolid as opaque hex in light mode", () => {
    expect(Colors.light.cardSolid).toMatch(/^#[0-9A-Fa-f]{6}$/);
  });
...
describe("Spacing", () => {
  ...
  it("should use 4pt grid (all values divisible by 2)", () => {
    Object.values(Spacing).forEach((value) => {
...
describe("FontSize", () => {
  it("should have all iOS26 tokens", () => {
  ...
  it("should have caption as the smallest", () => {
    const values = Object.values(FontSize) as number[];
    const min = Math.min(...values);
    expect(FontSize.caption).toBe(min);
  });

  it("should have title1 > title2 > title3", () => {
    expect(FontSize.title1).toBeGreaterThan(FontSize.title2);
    expect(FontSize.title2).toBeGreaterThan(FontSize.title3);
  });

  it("should have body between footnote and title3", () => {
    expect(FontSize.body).toBeGreaterThan(FontSize.footnote);
    expect(FontSize.body).toBeLessThan(FontSize.title3);
  });
});
...
describe("Shadow", () => {
  ...
  it("should have stronger shadow opacity in dark mode than light for each level", () => {
    ["sm", "md", "lg"].forEach((level) => {
      const lightLevel = Shadow.light[level as keyof typeof Shadow.light];
      const darkLevel = Shadow.dark[level as keyof typeof Shadow.dark];
  ...
  it("should have increasing elevation within each mode", () => {
    ["light", "dark"].forEach((mode) => {
      const m = Shadow[mode as keyof typeof Shadow];
      expect(m.md.elevation).toBeGreaterThan(m.sm.elevation);
      expect(m.lg.elevation).toBeGreaterThan(m.md.elevation);
    });
  });

  it("should have shadowOffset.height > 0 for all levels", () => {
    ["light", "dark"].forEach((mode) => {
      ["sm", "md", "lg"].forEach((level) => {
        const shadow =
          Shadow[mode as keyof typeof Shadow][
